...
<form action="<?php echo __SITEURL?>/users/profile" method="post">	
	...
	<div class="input-group">
		...
		<input name="email" <?php if($s["f_reg_required1"]=="on") echo "required"?> type="email" class="form-control" placeholder="<?php $language->dump("email")?>" aria-describedby="sizing-addon1" value="<?php echo $_SESSION['account']['email']?>">
	</div><br>			
	<div class="input-group">
		<span class="input-group-addon" id="sizing-addon1"><i class="fa fa-phone"></i></span>
		<input name="phone" pattern="^[0-9\+]{8,13}$" <?php if($s["f_reg_required2"]=="on") echo "required"?> type="text" class="form-control" placeholder="<?php $language->dump("phone")?>" aria-describedby="sizing-addon1" value="<?php echo $_SESSION['account']['phone']?>">
	</div><br>
