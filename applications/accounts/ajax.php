...
Accounts::authAccess(USER_AUTH_SU) or die;
($_POST["tf"] == "yes") or die;
...
switch (request(2)) {
	...
	case "changeName":
		Database::update(
			"app_users_list",
			(new DatabaseRowInput)->setField("name", $_POST["names"]),
			"id",
			$_POST["name"]
		) or exit;
		die($_POST["name"]);
	case "changeMail":
		...
		Database::update(
			"app_users_list",
			(new DatabaseRowInput)->setField("email", $_POST["mail"]),
			"id",
			$_POST["name"]
		) or die;
		die($_POST["name"]);
	case "changePhone":
		...
		Database::update(
			"app_users_list",
			(new DatabaseRowInput)->setField("phone", $_POST["phone"]),
			"id",
			$_POST["name"]
		) or die;
		die($_POST["name"]);
	case "changeLocal":
		Database::update(
			"app_users_list",
			(new DatabaseRowInput)->setField("lang", $_POST["local"]),
			"id",
			$_POST["name"]
		) or die;
		die($_POST["name"]);
	case "changeUname":
		...
		Database::update(
			"app_users_list",
			(new DatabaseRowInput)->setField("username", $_POST["uname"]),
			"id",
			$_POST["name"]
		) or die;
		die($_POST["name"]);
	case "changePass":
		...
		Database::update(
			"app_users_list",
			(new DatabaseRowInput)->setField("password", Accounts::hashPassword($_POST["paswd"])),
			"id",
			$_POST["name"]
		) or die;
		die($_POST["name"]);
	case "changeGroup":
		...
		Database::update(
			"app_users_list",
			(new DatabaseRowInput)->setField("group", $_POST["toGroup"]),
			"id",
			$_POST["uid"]
		) or die;
		die($_POST["uid"]);
