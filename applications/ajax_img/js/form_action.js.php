...
<script>
(function() {
	var resizeImage = (g,c) => {var h=new FileReader,d=new Image,e=document.createElement("canvas");return new Promise(function(m,n){g.type.match(/image.*/)?(h.onload=function(p){d.onload=function(){var b=d.width,a=d.height;b>a?b>c&&(a*=c/b,b=c):a>c&&(b*=c/a,a=c);e.width=b;e.height=a;e.getContext("2d").drawImage(d,0,0,b,a);a=e.toDataURL("image/png");b=0<=a.split(",")[0].indexOf("base64")?atob(a.split(",")[1]):unescape(a.split(",")[1]);a=a.split(",")[0].split(":")[1].split(";")[0];for(var k=b.length,l=new Uint8Array(k),f=0;f<k;f++)l[f]=b.charCodeAt(f);return m(new Blob([l],{type:a}))};d.src=p.target.result},h.readAsDataURL(g)):n(Error("Not an image"))})};
